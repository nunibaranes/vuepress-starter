...
export function resolveFooterItems (page, route, site, localePath) {
    const { pages, themeConfig } = site;
    ...
    const pageSidebarConfig = page.frontmatter.footer || localeConfig.footer || themeConfig.footer;
    if (pageSidebarConfig === 'auto') {
        return resolveHeaders(page);
    }

    const sidebarConfig = localeConfig.footer || themeConfig.footer;
    if (!sidebarConfig) {
        return []
    } else {
        const { base, config } = resolveMatchingConfig(route, sidebarConfig);
